#!/usr/bin/env python3
"""
华大MGI T7测序下机数据文件批量重命名脚本（批量处理版）
支持自动识别批量模式：若根目录无fq文件但有子文件夹，则自动处理所有子文件夹
根据Excel表格中的映射关系重命名fq文件，并去除_raw部分
同时更新每个子文件夹中的Excel表格文件Sheet1工作表中的样本条码信息
支持备份和格式保留
"""
...
def remove_raw_from_filename(filename):
    """从文件名中去除_raw部分（针对 fq 文件）"""
    return re.sub(r'_raw(?=_[12]\.fq\.gz)', '', filename)


def rename_fq_files(data_dir, mapping_dict, overwrite=False, dry_run=False):
    """重命名目录中的 fq 文件"""
    fq_files = []
...
def process_single_directory(data_dir, mapping_dict, sample_barcode_col, overwrite, dry_run):
    """处理单个目录：重命名fq文件并更新Excel文件"""
    print(f"\n{'='*60}")
    print(f"处理目录: {data_dir}")
    print('='*60)

    fq_renamed = rename_fq_files(data_dir, mapping_dict, overwrite, dry_run)
    excel_updated = update_excel_file(data_dir, mapping_dict, sample_barcode_col, dry_run)
...
def main():
    ...
    print("华大MGI T7测序下机数据文件批量重命名脚本（批量处理版）")
    print("=" * 70)
    ...
    for sub_dir in dirs_to_process:
        fq_count, excel_count = process_single_directory(
