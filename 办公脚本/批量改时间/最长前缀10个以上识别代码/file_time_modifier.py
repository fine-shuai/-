...
PREFIX_MATCH_THRESHOLD = 10   # 前缀匹配最少文件数，低于此值将跳过
...
def match_filename(filename, mapping, prefix_list):
    """返回 (mtime, ctime, atime) 和匹配类型，None表示无匹配"""
    # 完全匹配
...
def main():
    ...
    # --- 统计每个前缀（包括精确匹配）匹配到的文件列表 ---
    # 构建前缀->文件列表映射，同时记录该前缀对应的数据
    prefix_to_files = {}
    prefix_to_data = {}
    for fpath in all_files:
        ...
        # 先找完全匹配
        if fname in mapping:
            key = f"exact:{fname}"   # 精确匹配单独键
            if key not in prefix_to_files:
                prefix_to_files[key] = []
                prefix_to_data[key] = mapping[fname]
            prefix_to_files[key].append(fpath)
        else:
            # 前缀匹配
            best_prefix = None
            best_data = None
            best_len = -1
            for prefix, data in prefix_list:
                if fname.startswith(prefix) and len(prefix) > best_len:
                    best_len = len(prefix)
                    best_prefix = prefix
                    best_data = data
            if best_prefix:
                key = f"prefix:{best_prefix}"
                if key not in prefix_to_files:
                    prefix_to_files[key] = []
                    prefix_to_data[key] = best_data
                prefix_to_files[key].append(fpath)

    # --- 确定哪些前缀满足修改条件 ---
    # 条件：精确匹配（exact）无条件执行；前缀匹配要求文件数 >= PREFIX_MATCH_THRESHOLD
    eligible_keys = []
    for key, file_list in prefix_to_files.items():
        if key.startswith("exact:"):
            eligible_keys.append(key)
        else:
            if len(file_list) >= PREFIX_MATCH_THRESHOLD:
                eligible_keys.append(key)
            else:
                print(f"警告：前缀 '{key[7:]}' 仅匹配到 {len(file_list)} 个文件（不足 {PREFIX_MATCH_THRESHOLD}），将跳过修改。")
    ...
    for key in eligible_keys:
        file_list = prefix_to_files[key]
        mtime_str, ctime_str, atime_str = prefix_to_data[key]
