...
def main():
    ...
    mapping = read_excel_table(table_path)
    if mapping is None:
        input("按回车键退出...")
        sys.exit(1)
    print(f"已加载 {len(mapping)} 条记录。")
    print(f"表格中的文件名列表: {list(mapping.keys())}")
    ...
    for filepath in all_files:
        ...
        processed += 1
        mtime_str, ctime_str, atime_str = mapping[filename]
        print(f"\n处理文件: {filename}")
    ...
    if processed == 0:
        print("\n警告：没有找到与表格匹配的文件。")
        print("请确保表格中的文件名与实际文件名（包括扩展名）完全一致。")
        print("如果文件在子文件夹中，脚本会递归搜索所有文件。")
    elif dry_run:
        print("\n试运行结束。如需正式修改，请重新运行并选择正式模式。")
    else:
        print("\n所有文件处理完成。")
