...
if __name__ == "__main__":
    # 获取目标文件夹路径（命令行参数）
    if len(sys.argv) < 2:
        print("用法：python generate_xlsx.py <目标文件夹路径>")
        sys.exit(1)

    target_dir = sys.argv[1]

    # 定义输出 Excel 文件名（固定为“文件清单.xlsx”）
    output_file = os.path.join(target_dir, "文件清单.xlsx")
    ...
    if file_rows is not None:
        ...
        if not root_name:  # 如驱动器根目录
            root_name = target_dir
        # 按相对路径排序，使输出更有序
